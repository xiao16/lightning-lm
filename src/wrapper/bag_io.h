...
class RosbagIO {
   public:
    ...
    using LivoxCloud2Handle = std::function<bool(livox_ros_driver2::msg::CustomMsg::ConstPtr)>;
   ...
   private:
    std::map<std::string, PointCloud2Handle> cloud2_funcs_;
    std::map<std::string, LivoxCloud2Handle> livox_funcs_;
    std::map<std::string, ImuHandle> imu_funcs_;
