...
nav_msgs::OccupancyGrid G2P5Map::ToROS() {
    ...
    occu_map.info.resolution = static_cast<float>(options_.resolution_);
    occu_map.info.width = static_cast<uint32_t>(image_width);
    occu_map.info.height = static_cast<uint32_t>(image_height);
